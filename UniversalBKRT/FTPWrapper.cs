    ...
    class FTPWrapper
    {
        ...
        public async static Task OpenConnection(EBMFPSession eBMFPSession, string strFTPUsername, string strPassword)
        {
            try
            {
                ...
                if (eBMFPSession.FTPSSLEnabled == EState.ENABLED)
                {
                    ftpClient.EncryptionMode = FtpEncryptionMode.Implicit;
                    ftpClient.SslProtocols = SslProtocols.Tls | SslProtocols.Ssl2 | SslProtocols.Ssl3 | SslProtocols.Tls11 | SslProtocols.Tls12;
        ...
        private static void FtpClient_ValidateCertificate(FtpClient control, FtpSslValidationEventArgs e)
        {
            e.Accept = true;
        }
